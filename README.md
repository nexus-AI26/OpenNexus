...
```bash
sudo cp deploy/opennexus.service /etc/systemd/system/
...
- Application at `/opt/opennexus/`
- Virtual environment at `/opt/opennexus/venv/`
- A system user `opennexus` (create with `sudo useradd -r -s /bin/false opennexus`)
